...
class GossipingSpider(CrawlSpider):
    ...
    base_url = 'https://www.ptt.cc/bbs/{0}/index.html'
    rules = (
        Rule(LinkExtractor(allow=r'.*/bbs/.+/index\d+.html'), callback='parse_item', follow=True),
        Rule(LinkExtractor(allow=r'.*/bbs/.+/M\..+.html'), callback='parse_article', follow=False)
    )
    ...
    def _get_kanbans(self):
        for kanban in self.kanbans:
            self.start_urls.append(self.base_url.format(kanban))